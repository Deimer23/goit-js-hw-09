...
const ft = (0, _flatpickrDefault.default)(dataTime, options);
let SetTime;
start.disabled = true;
...
function addLeadingZero(value) {
    let cadena;
    if (value < 10) cadena = value.toString().padStart(2, "0 ");
    else cadena = value.toString();
    return cadena;
}
        ...
        if (cronometro.seconds == 0) {
            if (cronometro.minutes == 0) {
                if (cronometro.hours == 0) {
                    if (cronometro.days == 0) clearInterval(SetTime);
                    else {
                        cronometro.days -= 1;
                        days.innerHTML = addLeadingZero(cronometro.days);
                        cronometro.hours = 23;
